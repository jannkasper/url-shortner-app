...
passport.use(
    new JwtStrategy(jwtOptions, async (payload, done) => {
        try {
            const user = await query.default.user.find({email: payload.sub});
...
passport.use(
    new LocalStrategy(localOptions, async (email, password, done) => {
        try {
            const user = await query.default.user.find({ email });
            if (!user) {
...
passport.use(
    new LocalAPIKeyStrategy(localAPIKeyOptions, async (apikey, done) => {
        try {
            const user = await query.default.user.find({ apikey });
            if (!user) {
                return done(null, false);
            }
            return done(null, user);
