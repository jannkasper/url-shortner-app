const { isAfter, subDays, set } =  require("date-fns");
...
exports.add = async  (params) => {
    ...
    const visit = await knex.db("visits")
        .where({link_id: params.id})
        .andWhere(
            knex.db.raw("date_trunc('hour', created_at) = date_trunc('hour', ?)", [
                knex.db.fn.now()
            ])
        )
        .first();
...
exports.find = async (match, total) => {
    // if (match.link_id) {
    ...
    const visitsArray = await knex.db("visits")
        .where(match);
    const nowUTC = utils.getUTCDate();
    const now = new Date();

    for await (const visit of visitsArray) {
        utils.STATS_PERIODS.forEach(([days, type]) => {
            const isIncluded = isAfter(new Date(visit.created_at), subDays(nowUTC, days));
                ...
                stats[type].stats = {
                    ...
                    country: {
                        ...
                        ...Object.entries(visit.countries).reduce(
                            (obj, [country, count]) => ({...obj, [country]: (period.country[country] || 0) + count}), {})
                    },
                    referrer: {
                        ...period.referrer,
                        ...Object.entries(visit.referrers).reduce(
                            (obj, [referrer, count]) => ({...obj, [referrer]: (period.referrer[referrer] || 0) + count}), {})
                    }
                };
        ...
        stats.allTime.stats = {
            ...
            country: {
                ...allTime.country,
                ...Object.entries(visit.countries).reduce(
                    (obj, [country, count]) => ({...obj, [country]: (allTime.country[country] || 0) + count}), {})
            },
            referrer: {
                ...allTime.referrer,
                ...Object.entries(visit.referrers).reduce(
                    (obj, [referrer, count]) => ({...obj, [referrer]: (allTime.referrer[referrer] || 0) + count}), {})
            }
        };
